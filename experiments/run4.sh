...
for (( i=1; i<=$#; i++ )); do
  ...
  if [[ "$arg" == "--help" || "$arg" == "-h" ]]; then
    ...
    echo "Env overrides for HF path: TMAX_OVERRIDE, MAX_RUNTIME_TMAX, DEFAULT_SEQ_OVERRIDE"
    echo "Engine Options:"
...
EOF
  ...
  # Optional runtime overrides for the HF/native path.
  # This fixes the old behavior where hf_env.sh would overwrite a smaller
  # user-provided TMAX and then hard-cap only to 8192.
  if [[ -n "${DMODEL_OVERRIDE:-}" ]]; then DMODEL="$DMODEL_OVERRIDE"; fi
  if [[ -n "${NHEAD_OVERRIDE:-}" ]]; then NHEAD="$NHEAD_OVERRIDE"; fi
  if [[ -n "${NLAY_OVERRIDE:-}" ]]; then NLAY="$NLAY_OVERRIDE"; fi
  if [[ -n "${FFN_OVERRIDE:-}" ]]; then FFN="$FFN_OVERRIDE"; fi
  if [[ -n "${TMAX_OVERRIDE:-}" ]]; then
    echo "[*] Applying TMAX override: $TMAX -> $TMAX_OVERRIDE"
    TMAX="$TMAX_OVERRIDE"
  fi

  # Runtime cap remains configurable. Default stays conservative.
  MAX_RUNTIME_TMAX="${MAX_RUNTIME_TMAX:-8192}"
  if [[ "$TMAX" -gt "$MAX_RUNTIME_TMAX" ]]; then
    echo "[*] Capping TMAX from $TMAX to $MAX_RUNTIME_TMAX (VRAM-safe)"
    TMAX="$MAX_RUNTIME_TMAX"
  fi

  # Very small GPUs often still need a lower default seq for chat.
  : "${DEFAULT_SEQ_OVERRIDE:=}"
  if [[ -n "$DEFAULT_SEQ_OVERRIDE" ]]; then
    found_seq=0
    for (( j=0; j<${#PASSED_ARGS[@]}; j++ )); do
      if [[ "${PASSED_ARGS[$j]}" == "--seq" ]]; then found_seq=1; break; fi
    done
    if [[ $found_seq -eq 0 ]]; then
      PASSED_ARGS+=("--seq" "$DEFAULT_SEQ_OVERRIDE")
      echo "[*] Injecting default --seq $DEFAULT_SEQ_OVERRIDE"
    fi
  fi
else
