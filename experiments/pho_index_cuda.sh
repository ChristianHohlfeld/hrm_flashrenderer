...
# PHO-only index-backed CUDA prototype
# ------------------------------------
# Self-contained shell script that:
#   1) writes a single-file CUDA program
#   2) builds it with nvcc
#   3) creates/uses a tiny SSD-backed mmap index
#   4) runs either:
#        --build   : build demo index file
#        --server  : query -> return PHO/Index IDs only (no text resolve)
#        --client  : resolve PHO IDs locally using a static table
#
# Notes:
# - This is a bounded prototype for the architecture you described:
#     server stays in PHO/index space, client resolves.
# - It is NOT a drop-in replacement for your existing repo/model stack.
# - It keeps VRAM use tiny: only candidate slices are copied to the GPU.

MODE="--server"
WORKDIR="${WORKDIR:-$PWD}"
BIN="${BIN:-pho_server}"
CU="${CU:-pho_server.cu}"
INDEX="${INDEX:-pho_demo.index}"
TOPK="${TOPK:-8}"
CAND_LIMIT="${CAND_LIMIT:-256}"
...
if [[ ! -f "$WORKDIR/$INDEX" ]]; then
  echo "[*] No index found. Building demo index..."
  "$WORKDIR/$BIN" --build --index "$WORKDIR/$INDEX"
fi
...
case "$MODE" in
  --build)
    ...
    ;;
  --server)
    "$WORKDIR/$BIN" --server --index "$WORKDIR/$INDEX" --topk "$TOPK" --cand-limit "$CAND_LIMIT"
    ;;
  --client)
    "$WORKDIR/$BIN" --client
    ;;
