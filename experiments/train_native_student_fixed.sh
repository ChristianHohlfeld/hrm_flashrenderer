...
# IMPORTANT: also export the names run_llm_orig.sh actually uses
export DMODEL="$D"
export NHEAD="$H"
export NLAY="$L"
export FFN="$F"
...
PYFILE="$OUT_DIR/train_native_student_v2.py"
...
class Student:
    ...
    def train_example(self, prompt_ids, target_ids, lr):
        ...
        for tid in used:
            ...
            e_t = self.E[tid]
            dpre = dh * (1.0 - h * h)
            prompt_mean = np.mean(self.E[np.asarray(prompt_ids, dtype=np.int32)], axis=0)
            dW1 += np.outer(prompt_mean + 0.05 * e_t, dpre)
            db1 += dpre
            dE_acc += self.W1 @ dpre
        ...
        self.Wo -= lr * dWo / denom
        self.bo -= lr * dbo / denom
        self.W1 -= lr * dW1 / denom
        self.b1 -= lr * db1 / denom
        for pid in prompt_ids:
            self.E[pid] -= lr * dE_acc / max(1, len(prompt_ids) * denom)
        return loss
...
def eval_student(stu, split_rows):
    ...
    return {"nll": float(np.mean(losses)), "overlap": float(np.mean(ovs)), "score": float(np.mean(ovs) * 100.0 - np.mean(losses) * 10.0)}
...
export WORKDIR CALIB_JSONL INDEX_BIN OUT_DIR RUNNER D H L F TMAX DMODEL NHEAD NLAY FFN PAIR_K PAIR_K1 EPOCHS LR HOLDOUT_FRACTION CANDIDATES SEED GPUS
...
if [[ "${AUTO_CHAT:-1}" == "1" ]]; then
  ...
  exec env -u DMODEL -u NHEAD -u NLAY -u FFN -u TMAX \
    DMODEL="$D" NHEAD="$H" NLAY="$L" FFN="$F" TMAX="$TMAX" \
    "${CMD[@]}"
else
