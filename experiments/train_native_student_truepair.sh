...
# train_native_student_truepair.sh
# Train bounded student on TRUE PairIndex token IDs from index_v7,
# not on fake hashed PHO ids.
#
# Flow:
#   1) optionally rebuild real prompt/response dump
#   2) load index_v7 true tokenizer (stage1 + stage2 hash)
#   3) encode prompt/teacher_text into true pair IDs
#   4) train bounded student on those IDs
#   5) export native checkpoints
#   6) optionally run llm_engine_full_blast.sh with best checkpoint
#
# Files expected next to this script:
#   - llm_engine_full_blast.sh
#   - index_v7_k18192_k28192.bin
#
# Defaults are moderate but real.
...
HIDDEN="${HIDDEN:-256}"
TEACHER_LEN="${TEACHER_LEN:-48}"
SEED="${SEED:-1234}"
AUTO_CHAT="${AUTO_CHAT:-1}"
GPU="${GPU:-1}"

OUT_DIR="${OUT_DIR:-$WORKDIR/native_student_truepair}"
mkdir -p "$OUT_DIR"
...
if [[ ! -f "$RUNTIME_SH" ]]; then
  echo "FATAL: RUNTIME_SH not found: $RUNTIME_SH"
  exit 1
fi

MAKE_DUMP_PY="$OUT_DIR/make_calib_dump.py"
TRAIN_PY="$OUT_DIR/train_truepair.py"
...
for row in ds:
    ...
    if not prompt or not resp:
        continue
    if len(prompt) < 2 or len(resp) < 2:
        continue
    if len(prompt) > 4000 or len(resp) > 4000:
        continue
    items.append({"prompt": prompt, "teacher_text": resp})

if not items:
    raise SystemExit("No usable items extracted")
...
def stage2_lookup(pi: PairIndex, a: int, b: int):
    ...
    while True:
        hk = pi.hkeys[i]
        if hk == key:
            return BASE_V + pi.k1 + pi.hvals[i]
        if hk == 0:
            return None
        i = (i + 1) & mask
        if i == start:
...
def encode_ids(pi: PairIndex, b: bytes):
    ...
    # repeatedly merge using stage2 until stable
    changed = True
...
with CALIB_JSONL.open("r", encoding="utf-8") as f:
    for line in f:
        ...
        if not prompt or not teacher_text:
            continue
        p = encode_ids(pi, prompt.encode("utf-8", errors="replace"))
        t = encode_ids(pi, teacher_text.encode("utf-8", errors="replace"))
        # keep bounded target length
        p = p[:64]
        t = t[:TEACHER_LEN]
...
if [[ ! -f "$CALIB_JSONL" || "$FORCE_REBUILD_CALIB" == "1" ]]; then
  ...
  DATASET_ID="$DATASET_ID" DATASET_SPLIT="$DATASET_SPLIT" CALIB_LIMIT="$CALIB_LIMIT" CALIB_JSONL="$CALIB_JSONL" python3 "$MAKE_DUMP_PY"
else
  echo "[*] Reusing existing CALIB_JSONL: $CALIB_JSONL"
fi

echo "[*] Running true-pair native student training..."
...
if [[ "$AUTO_CHAT" == "1" ]]; then
  echo "[*] Launching stable runtime with best checkpoint..."
  exec env -u DMODEL -u NHEAD -u NLAY -u FFN -u TMAX \
    CKPT="$BEST_CKPT" GPU="$GPU" "$RUNTIME_SH"
else
  echo "[*] AUTO_CHAT=0; training complete."
fi
