...
# =============================================================================
# Local DeepSeek via vLLM / OpenAI-compatible API - Drop-in Wrapper Mode
# =============================================================================
DO_LOCAL=0
LOCAL_MODEL=""
LOCAL_PROMPT=""
LOCAL_URL="${LOCAL_URL:-http://localhost:8000/v1}"
LOCAL_API_KEY="${LOCAL_API_KEY:-EMPTY}"   # vLLM braucht oft keinen Key, aber OpenAI-kompatibel
...
# Extract arguments specifically for Bash, pass remainder to C++ engine
skip_next=0
for (( i=1; i<=$#; i++ )); do
...
url = os.environ["LOCAL_URL"].rstrip("/") + "/models"
try:
    urllib.request.urlopen(url, timeout=2).read()
except Exception as e:
    print(f"FATAL: vLLM server not reachable at {url} ({e})")
    sys.exit(1)
...
def get_t(name, expected_shape=None):
    ...
    if expected_shape and list(t.shape) != list(expected_shape):
        ...
        # Pad or truncate (dumb mapping to force hardware to run)
...
def write_tensor(f, t):
    # Scale and cast to deterministic int32 signature
    t_int = (t * 1000.0).to(torch.int32).flatten().numpy()
    f.write(t_int.tobytes())

# Parse index logic to get magic header length
...
with open(ckpt_path, "wb") as f:
    ...
    # 2. Write Tokenizer data directly from index_v7.bin (skipping its 24-byte header)
    f.write(index_data[24:])
    
    # 3. Write Weights to custom layout
    ...
    for l in range(L):
        ...
        # Mapped from QKV to customized shapes
...
# Generate LUT header for deterministic fixed-point exponential
python3 - << 'PY' > "$TMP_CU_DIR/softmax_lut.h"
import math

def build_lut():
    # 4097 entries to cover Q8 dx from -16 to 0, plus boundary safety
    DX_MIN = -16.0
    ENTRIES = 4096
    
    out = ["#pragma once\n", "#include <cstdint>\n\n"]
    out.append("#ifdef __CUDACC__\n")
    out.append("__constant__ int32_t c_exp_lut[4097] = {\n")
    
    for i in range(ENTRIES + 1):
        # i goes from 0 to 4096 => dx from -16 to 0
        dx = DX_MIN + (i / ENTRIES) * abs(DX_MIN)
        val = math.exp(dx)
        # convert to Q30
        q30 = int(round(val * (1<<30)))
        # clamp
        if q30 > (1<<30): q30 = (1<<30)
        if q30 < 0: q30 = 0
        
        out.append(f"  {q30},  // i={i}, dx={dx:.4f}\n")
...
// Softmax fixed-point contract
// logits_q = (dp4a_logits >> SM_SHIFT)  (int32)
// dx clamped to [-16,0] in Q8 (step 1/256)
static constexpr int SM_SHIFT = 10;   // tune once, then lock it
static constexpr int DX_Q = 8;        // Q8 for dx
static constexpr int DX_MIN_Q = -(16 << DX_Q);
static constexpr int EXP_Q = 30;     // Q30
static constexpr int PROB_Q = 30;           // probabilities in Q30
static constexpr int DY_Q = 24;             // gradient scale output (optional)

__device__ __forceinline__ int32_t exp_dx_q30(int32_t dx_q8){
  // dx_q8 in [-4096,0] (=-16..0 in Q8)
  if(dx_q8 <= DX_MIN_Q) return c_exp_lut[0];
  if(dx_q8 >= 0)        return c_exp_lut[4096];
  // map [-16..0] Q8 to [0..4096]
  int idx = (dx_q8 - DX_MIN_Q); // [-4096..0] - (-4096) => [0..4096]
  // dx_q8 increments by 1 => step 1/256 => 4096 span -> perfect
  return c_exp_lut[idx];
}

__device__ __forceinline__ int32_t dp4a_dot_i8(const int8_t* x, const int8_t* wrow, int K){
  // K multiple of 4
  int32_t acc=0;
  for(int k=0;k<K;k+=4){
    int32_t a = *((const int32_t*)(x + k));
...
__global__ void xent_dlogits_int(int32_t* dlogits, int32_t* loss, const int32_t* logits, const uint16_t* tgt, int N, int32_t invN){
  int n=blockIdx.x; if(n>=N) return;
  int y=(int)tgt[n];
  for(int v=threadIdx.x; v<V; v+=blockDim.x){
    dlogits[(size_t)n*(size_t)Vpad+(size_t)v] = (v==y) ? -invN : 0;
  }
  if(threadIdx.x==0) loss[n] = 1024;
}
...
__global__ void init_row_ms(int32_t* row_max_q, uint64_t* row_sum_q30, int N){
  int n = blockIdx.x*blockDim.x + threadIdx.x;
  if(n<N){
    row_max_q[n] = -2147483648;
    row_sum_q30[n] = 0;
  }
}

__global__ void softmax_online_passA_dp4a(
    int32_t* row_max_q, uint64_t* row_sum_q30,
    const int8_t* X,              // [N,D] int8 (your Xnorm_h)
    const int8_t* Wtr_chunk,      // [VCHUNK,D] int8  (points into Wout_tr + v0*D)
    int N)
{
  ...
  // one row per block
  const int8_t* x = X + (size_t)n*(size_t)D;

  // compute chunk max in logits_q domain
  int32_t tmax = -2147483648;

  for(int j=threadIdx.x; j<VCHUNK; j+=blockDim.x){
    const int8_t* wrow = Wtr_chunk + (size_t)j*(size_t)D;
    int32_t l = dp4a_dot_i8(x, wrow, D);
    int32_t q = (l >> SM_SHIFT);
    tmax = max(tmax, q);
  }

  // warp/block reduce max
  __shared__ int32_t smax[8]; // 256 threads => 8 warps
  ...
  __syncthreads();
  if(wid==0){
    int32_t v = (lane < (blockDim.x>>5)) ? smax[lane] : -2147483648;
    for(int off=16; off>0; off>>=1){
      int32_t u = __shfl_xor_sync(0xFFFFFFFFu, v, off);
      v = max(v,u);
    }
    if(lane==0) smax[0]=v;
  }
  __syncthreads();
  int32_t chunk_max_q = smax[0];

  // compute chunk sumexp relative to chunk_max_q
  uint64_t sum = 0;
  for(int j=threadIdx.x; j<VCHUNK; j+=blockDim.x){
    const int8_t* wrow = Wtr_chunk + (size_t)j*(size_t)D;
    int32_t l = dp4a_dot_i8(x, wrow, D);
    ...
    int32_t dx_q8 = (q - chunk_max_q) << DX_Q; // <=0
    if(dx_q8 < DX_MIN_Q) dx_q8 = DX_MIN_Q;
    if(dx_q8 > 0) dx_q8 = 0;

    sum += (uint64_t)(uint32_t)exp_dx_q30(dx_q8);
  }

  // reduce sum (uint64) deterministically
  __shared__ uint64_t ssum[256];
  ...
  uint64_t chunk_sum_q30 = ssum[0];

  // online update with global row (single thread)
  if(threadIdx.x==0){
    int32_t m_old = row_max_q[n];
    uint64_t s_old = row_sum_q30[n];

    int32_t m_new = (m_old > chunk_max_q) ? m_old : chunk_max_q;

    // scale old sum to new max: s_old * exp(m_old - m_new)
    uint64_t s_old_scaled = 0;
    if(s_old){
      int32_t d = (m_old - m_new) << DX_Q; // <=0
      if(d < DX_MIN_Q) d = DX_MIN_Q;
      if(d > 0) d = 0;
      uint64_t e = (uint64_t)(uint32_t)exp_dx_q30(d);
      // Q30 * Q30 -> Q60, keep Q30
      s_old_scaled = (s_old * e) >> EXP_Q;
    }

    // scale chunk sum similarly: chunk_sum * exp(chunk_max - m_new)
    int32_t d2 = (chunk_max_q - m_new) << DX_Q;
    if(d2 < DX_MIN_Q) d2 = DX_MIN_Q;
    if(d2 > 0) d2 = 0;
    uint64_t e2 = (uint64_t)(uint32_t)exp_dx_q30(d2);
    uint64_t chunk_scaled = (chunk_sum_q30 * e2) >> EXP_Q;

    row_max_q[n] = m_new;
    row_sum_q30[n] = s_old_scaled + chunk_scaled;
  }
}

__global__ void softmax_passB_dy_i8_dp4a(
    int8_t* dY_i8_row,   // [N,VCHUNK]
    int8_t* dY_i8_tr,    // [VCHUNK,N]
    int32_t* loss,       // [N] optional
    const int32_t* row_max_q,
    const uint64_t* row_sum_q30,
    const int8_t* X,           // [N,D]
    const int8_t* Wtr_chunk,   // [VCHUNK,D]
    const uint16_t* tgt,       // [N]
    int N, int v0, int Mvalid)
{
  int n = (int)blockIdx.x;
  if(n>=N) return;

  const int8_t* x = X + (size_t)n*(size_t)D;
  int32_t m = row_max_q[n];
  uint64_t s = row_sum_q30[n];
  if(s==0) s = 1;

  // inv_sum in Q32 once per row
  __shared__ uint32_t inv_q32;
  if(threadIdx.x==0){
    inv_q32 = (uint32_t)(((unsigned long long)1<<32) / (unsigned long long)s);
  }
  __syncthreads();
  ...
  for(int j=threadIdx.x; j<VCHUNK; j+=blockDim.x){
    int v = v0 + j;
    if(j >= Mvalid || v >= V){
      // write zeros for out-of-range to keep dp4a stable
      dY_i8_row[(size_t)n*(size_t)VCHUNK + (size_t)j] = 0;
      dY_i8_tr[(size_t)j*(size_t)N + (size_t)n] = 0;
      continue;
    }
    ...
    int32_t dx_q8 = (q - m) << DX_Q;
    if(dx_q8 < DX_MIN_Q) dx_q8 = DX_MIN_Q;
    if(dx_q8 > 0) dx_q8 = 0;

    uint32_t e = (uint32_t)exp_dx_q30(dx_q8); // Q30
    // p = e / s  (Q30). We have inv_q32 ~= 1/s.
    // p_q30 = (e * inv_q32) >> 32
    uint32_t p_q30 = (uint32_t)(((unsigned long long)e * (unsigned long long)inv_q32) >> 32);

    // dy = p - 1{v==y}, keep in Q30 then downshift to i8 for dp4a
    int32_t dy_q30 = (int32_t)p_q30 - ((v==y) ? (1<<30) : 0);

    // scale to i8: choose SHIFT so typical dy maps into [-127..127]
    // Here: dy_i8 = dy_q30 >> 23  -> about Q7 range
    int32_t dy_i8 = dy_q30 >> 23;
    if(dy_i8 > 127) dy_i8 = 127;
    if(dy_i8 < -128) dy_i8 = -128;

    int8_t out = (int8_t)dy_i8;
    dY_i8_row[(size_t)n*(size_t)VCHUNK + (size_t)j] = out;
    dY_i8_tr[(size_t)j*(size_t)N + (size_t)n] = out;

    if(v==y && threadIdx.x==0){
      uint32_t px = p_q30;
      int32_t l_q10 = 0;
      if (px == 0) {
          l_q10 = 20 * 1024;
          ...
          int lz = __clz(px);
          int msb = 31 - lz;
          uint32_t frac = (px << (lz + 1)) >> 22; // top 10 bits
          int32_t lnx = msb * 710 + ((frac * 710) >> 10);
          l_q10 = (30 * 710) - lnx;
          if (l_q10 < 0) l_q10 = 0;
      }
      loss[n] = l_q10;
    } else if ((unsigned)y >= (unsigned)V && j == 0 && threadIdx.x == 0) {
      loss[n] = 50 * 1024;
    }
  }
}

static inline void dp4a_gemm_dA_i8(int32_t* dX, const int8_t* dY_i8_row, const int8_t* Wrm, int N, int M, int K){
  // dY_i8_row: [N,M], Wrm: [K,M] (row-major KxM), output dX: [N,K]
  dim3 blk(16,16);
  dim3 grd((K+15)/16, (N+15)/16);
  dp4a_gemm_kernel<<<grd, blk>>>(dX, dY_i8_row, Wrm, N, K, M);
  KERNEL_CHECK();
}

static inline void dp4a_gemm_dW_tr_i8(int32_t* dW, int8_t* Atr, const int32_t* A, const int8_t* dYtr,
                                     int N, int K, int M){
  // Atr = A^T in i8 [K,N]
  dim3 blk(16,16);
  build_Atr_i8<<<dim3((N+15)/16,(K+15)/16), blk>>>(Atr, A, N, K); KERNEL_CHECK();
  // Now dp4a GEMM: dW[K,M] = Atr[K,N] * dYtr[M,N]^T? We want KxM output
  // Our dp4a kernel expects A[N,K] and Btr[M,K]. But we already use kernel for generic C[row*M+col].
  // We can call dp4a_gemm_kernel with:
  //   row=K, col=M, Kdim=N, A=Atr (KxN), Btr=dYtr (MxN)
  dim3 grd((M+15)/16, (K+15)/16);
  dp4a_gemm_kernel<<<grd, blk>>>(dW, Atr, dYtr, K, M, N);
  KERNEL_CHECK();
}
...
struct GPU {
  ...
  int8_t  *Xnorm_h;
  int32_t *Loss;
  int32_t *row_max,*chunk_max,*chunk_sum;
  uint64_t *row_sum;
...
static void gpu_alloc(GPU* g,int dev,int B,int T){
  ...
  CUDA_CHECK(cudaMalloc(&g->row_sum, (size_t)g->N*sizeof(uint64_t)));
...
static void train_step_device(GPU* g){
  ...
  // --- NEW: deterministic 2-pass online softmax (no logits_chunk) ---
init_row_ms<<<(N+255)/256,256>>>(g->row_max, (uint64_t*)g->row_sum, N); KERNEL_CHECK();
// NOTE: you currently have row_sum as int32*. You must change g->row_sum allocation to uint64_t.
// Allocate row_sum as uint64_t* (N entries) in gpu_alloc.

for(int v0=0; v0<V; v0+=VCHUNK){
  int Mvalid = (v0+VCHUNK<=V)? VCHUNK : (V - v0);
  (void)Mvalid;
  const int8_t* Wtr = g->Hw.Wout_tr + (size_t)v0*(size_t)D;
  softmax_online_passA_dp4a<<<N,256>>>(g->row_max, (uint64_t*)g->row_sum, g->Xnorm_h, Wtr, N);
  KERNEL_CHECK();
}
...
for(int v0=0; v0<V; v0+=VCHUNK){
  ...
  // write dY_i8_row into scratchHalf_head (reuse it) and dY_i8_tr into dYtr (reuse it)
  softmax_passB_dy_i8_dp4a<<<N,256>>>(
      (int8_t*)g->scratchHalf_head,   // dY row i8
      (int8_t*)g->dYtr,               // dY tr i8 (ensure allocated as int8_t*)
      g->Loss,
      g->row_max,
      (uint64_t*)g->row_sum,
      g->Xnorm_h,
  ...
  // dWout chunk: use already-transposed dy
  dp4a_gemm_dW_tr_i8(g->dWout_chunk, g->Atr, g->Xnorm, (int8_t*)g->dYtr, N, D, VCHUNK);
  scatter_add_dwout_chunk_int<<<(D*VCHUNK+255)/256,256>>>(g->G.Wout, g->dWout_chunk, v0); KERNEL_CHECK();

  // dXnorm += dY * Wout_rm_chunk   (but now dY is i8 row-major)
  pack_wout_rm_chunk_i8<<<(D*VCHUNK+255)/256,256>>>(g->Wout_rm_chunk, g->Hw.Wout_rm, v0); KERNEL_CHECK();
  dp4a_gemm_dA_i8(g->dXfull, (int8_t*)g->scratchHalf_head, g->Wout_rm_chunk, N, VCHUNK, D);
...
__global__ void kv_store_int8(int8_t* Kc, int8_t* Vc, const int32_t* k, const int32_t* v, int t){
  int i=blockIdx.x*blockDim.x+threadIdx.x;
  if(i<Dhf){
...
__global__ void chat_sample_kernel(const int32_t* logits, int* out_tok, int32_t temp_q16, uint32_t rand_val, int V, cudaTextureObject_t exp_tex) {
  int tid = threadIdx.x;
...
static int chat_step(ChatCtx* c, int t, int tok_id, int32_t temp_q16, uint32_t rand_val){
  ...
  for(int l=0;l<L;l++){
    ...
    kv_store_int8<<<(Dhf+255)/256,256>>>(c->Kc[l], c->Vc[l], c->k, c->v, t); KERNEL_CHECK();
    attn_decode_one_int8<<<H,32>>>(c->o, c->q, c->Kc[l], c->Vc[l], t); KERNEL_CHECK();
  ...
  if(temp_q16 <= 0){
    ...
    chat_sample_kernel<<<1, 32>>>(c->logits, c->amaxi, temp_q16, rand_val, V, c->exp_tex);
    KERNEL_CHECK();
    CUDA_CHECK(cudaMemcpy(&next, c->amaxi, sizeof(int), cudaMemcpyDeviceToHost));
  }
...
static void chat_repl(const PairIndex& pi, const std::vector<int32_t>& hostW, const char* ckpt_path, const char* prompt, bool do_measure, int32_t temp_q16, uint64_t seed){
  CUDA_CHECK(cudaSetDevice(0));
  GPU g0{};
  gpu_alloc(&g0, 0, 1, 16);
  ...
  int next=-1;
  for(uint16_t id: pre){
    next = chat_step(&ctx, t, (int)id, 0, 0);
    t++;
    if(t>=Tmax) break;
  }
  ...
  while(true){
    ...
    for(size_t i=0;i<ids.size();i++){
      next = chat_step(&ctx, t, (int)ids[i], 0, 0);
      t++;
      if(t>=Tmax) break;
    }
    if(t>=Tmax){ std::cout << "\n[ctx full]\n"; continue; }
...
int main(int argc,char** argv){
  ...
  int steps=2000,batch=8,seq=128,gpus_req=2;
  int32_t lr=5, wd=1, clip=5;
  ...
  if(!do_train && !do_chat) do_train=true;
  if(do_chat){
    chat_repl(pi, hostW, ckpt_path, chat_prompt, do_measure, temp_q16, seed);
    return 0;
  }
  ...
  for(int step=1; step<=steps; step++){
    ...
    if(log_every>0 && (step%log_every)==0){
      double Lm=0.0; for(int i=0;i<G;i++) Lm += (double)losses[(size_t)i]; 
      double mean_loss = Lm / (double)(G * batch * seq * 1024);
      if(do_measure){
        auto t1 = std::chrono::high_resolution_clock::now();
        double elapsed = std::chrono::duration<double>(t1 - t0).count();
        double tok_sec = (double)(log_every * G * batch * seq) / elapsed;
        t0 = t1; 
        std::printf("step %d/%d loss=%.6f ppl=%.3f tok/s=%.1f\n", step, steps, (float)mean_loss, (float)std::exp(mean_loss), tok_sec);
      } else {
        std::printf("step %d/%d loss=%.6f ppl=%.3f\n", step, steps, (float)mean_loss, (float)std::exp(mean_loss));
      }
      std::fflush(stdout);
    }
