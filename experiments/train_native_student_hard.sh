...
# train_native_student_hard.sh
# Harder bounded-student trainer for the stable runtime.
#
# One-shot flow:
#   1) optionally build/rebuild public prompt/response calibration dump
#   2) train a stronger bounded student (numpy only)
#   3) export native checkpoints
#   4) score candidates more aggressively
#   5) optionally launch llm_engine_full_blast.sh with best checkpoint
#
...
#
# Optional env:
#   DATASET_ID, DATASET_SPLIT, CALIB_LIMIT, FORCE_REBUILD_CALIB
#   EPOCHS, LR, CANDIDATES, HIDDEN, AUTO_CHAT, GPU
#
# Defaults are intentionally stronger than previous scripts.
...
# harder training defaults
EPOCHS="${EPOCHS:-24}"
LR="${LR:-0.02}"
HOLDOUT_FRACTION="${HOLDOUT_FRACTION:-0.1}"
CANDIDATES="${CANDIDATES:-8}"
HIDDEN="${HIDDEN:-256}"
TEACHER_LEN="${TEACHER_LEN:-32}"
SEED="${SEED:-1234}"
AUTO_CHAT="${AUTO_CHAT:-1}"
GPU="${GPU:-1}"

OUT_DIR="${OUT_DIR:-$WORKDIR/native_student_hard}"
mkdir -p "$OUT_DIR"
...
TRAIN_PY="$OUT_DIR/train_hard.py"
...
echo "[*] Running harder native student training..."
