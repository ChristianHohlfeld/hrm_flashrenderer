...
# run6_native_rc_v2.sh
# Fixes:
#  - hard-reset target dims so stale hf_env.sh exports cannot poison the run
#  - robust HF tensor key alias lookup (embed/norm/lm_head + attention/mlp variants)
#  - stays zero-torch / zero-accelerate / zero-llama.cpp
...
# HARD RESET — do not inherit poisoned env from hf_env.sh
export D=256
export H=8
export L=6
export F=1024
...
export WORKDIR DISTILL_DIR HF_MODEL RUNNER INDEX_BIN PAIR_K PAIR_K1 GPUS
export D H L F TMAX ENV_FILE CKPT_FILE MANIFEST_FILE

echo "[*] Running RC v2 zero-torch converter..."
python3 "$CONVERTER"
